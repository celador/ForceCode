		...
		return new Promise((resolve, reject) => {
			vscode.window.showQuickPick(options, config).then((edition: vscode.QuickPickItem) => {
				...
				vscode.window.showQuickPick(durOpts, config).then((duration: vscode.QuickPickItem) => {
					...
					vscode.window.showQuickPick(sDataOptions, config).then((sampleData: vscode.QuickPickItem) => {
						if (!sampleData) {
							return Promise.resolve();
						}
						...
						const theOptions: string = '--durationdays ' + duration.label + ' --definitionjson ' + JSON.stringify(optsObj);
						return dxService.createScratchOrg(theOptions)
							.then(res => {
