...
export default function bulkLoader(): Promise<any> {
    ...
    const panel = vscode.window.createWebviewPanel('fcBulk', "ForceCode Bulk Loader", vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    panel.webview.onDidReceiveMessage(message => {
        if (message.uploadCSV) {
            vscode.window.showOpenDialog({ filters: { 'CSV': ['csv'] }, defaultUri: defaultURI, canSelectFolders: false, canSelectMany: false }).then(uri => {
                if (uri) {
            ...
            batch = vscode.window.forceCode.conn.bulk.load(message.object, message.operation, csvFileIn, (err, rets) => {
                if(timeOut) {
                ...
                if(totalErrors > 0) {
                    vscode.window.showSaveDialog({ filters: { 'CSV': ['csv'] }, defaultUri: defaultURI, saveLabel: 'Save Error File' }).then(uri => {
                        if(uri) {
                            fs.outputFileSync(uri.fsPath, errorFile);
                        }
                    });
                }
                panel.webview.postMessage({ state: 'Completed', processed: totalRecords,
        ...
        timeOut = setTimeout(() => {
            try {
                batch.check().then(res => {
                    panel.webview.postMessage({ state: res.state, processed: res.numberRecordsProcessed,
