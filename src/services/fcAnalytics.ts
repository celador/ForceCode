...
export function getPreviousUUID(fcExtPath): boolean {
  ...
  contents.forEach(folder => {
    if (
      folder.indexOf('johnaaronnelson.forcecode-') !== -1 &&
  ...
  if (toCheck.length > 0) {
    ...
    const lastVersion: string = toCheck.reduce((ver1, ver2) => {
      ...
      const ver1Parts: number[] = ver1String
        .split('-')[0]
        .split('.')
        .map(part => parseInt(part));
      const ver2Parts: number[] = ver2String
        .split('-')[0]
        .split('.')
        .map(part => parseInt(part));
    ...
    if (fs.existsSync(path.join(extensionsPath, lastVersion, 'analytics.json'))) {
      fs.copyFileSync(
        path.join(extensionsPath, lastVersion, 'analytics.json'),
        path.join(fcExtPath, 'analytics.json')
      );
      return true;
    }
  }
